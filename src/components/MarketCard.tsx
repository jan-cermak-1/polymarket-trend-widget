...
export const MarketCard: React.FC<MarketCardProps> = ({ event, isTopItem = false }) => {
  ...
  if (isTopItem) {
    return (
      <div ref={cardRef} className="group relative p-2.5 border border-gray-200 dark:border-gray-700 rounded-lg bg-gradient-to-br from-white to-gray-50/50 dark:from-gray-800 dark:to-gray-900 hover:shadow-md transition-shadow flex flex-col h-full cursor-pointer">
        ...
        <div className={clsx(
          "absolute left-1/2 -translate-x-1/2 w-96 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 p-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-[9999] pointer-events-none transform",
          showTooltipBelow 
  ...
  return (
    <div ref={cardRef} className="group relative flex items-center gap-2 py-1.5 px-2 hover:bg-gray-50 dark:hover:bg-gray-800 rounded transition-colors cursor-pointer">
            ...
            <div className="flex gap-3 mb-3">
                {event.image && (
                    <img 
                        src={event.image} 
                        alt={event.title} 
                        className="w-10 h-10 rounded-md object-cover bg-gray-100 dark:bg-gray-700 border border-gray-200 dark:border-gray-700"
                    />
                )}
                <div className="flex-1">
                    <h4 className="font-semibold text-sm text-gray-900 dark:text-white leading-tight mb-1">
